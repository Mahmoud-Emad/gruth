...
pub fn run_sync(root: &Path, max_depth: usize, excluded: &[String]) -> Result<()> {
    ...
    println!(
        "\x1b[90m  Found {} repositories\x1b[0m",
        repos.len()
    );
    println!();
    ...
    for repo_path in &repos {
        ...
        if let Err(e) = git_ops::fetch_all_remotes(repo_path) {
            println!(
                "\x1b[31m  ✗ FETCH\x1b[0m  {} \x1b[90m({})\x1b[0m",
                display, e
            );
            errors += 1;
            continue;
        ...
        let info = match git_ops::get_repo_info(repo_path) {
            ...
            Err(e) => {
                println!(
                    "\x1b[31m  ✗ ERR  \x1b[0m  {} \x1b[90m({})\x1b[0m",
                    display, e
                );
                errors += 1;
                continue;
        ...
        match info.status {
            RepoStatus::Dirty | RepoStatus::Conflicts => {
                let label = if info.status == RepoStatus::Conflicts {
                    "conflicts"
                } else {
                    "dirty"
                };
                println!(
                    "\x1b[33m  ⊘ SKIP \x1b[0m  {} \x1b[90m[{}] ({})\x1b[0m",
                    display, info.branch, label
                );
                skipped += 1;
            }
            RepoStatus::Clean => {
                if info.behind == 0 {
                    println!(
                        "\x1b[32m  ✓ OK   \x1b[0m  {} \x1b[90m[{}]\x1b[0m",
                        display, info.branch
                    );
                    up_to_date += 1;
                } else {
                    match git_ops::pull_current_branch(repo_path) {
                        Ok(msg) => {
                            println!(
                                "\x1b[36m  ↓ PULL \x1b[0m  {} \x1b[90m[{}] (↓{} — {})\x1b[0m",
                                display, info.branch, info.behind, msg
                            );
                            synced += 1;
                        }
                        Err(e) => {
                            println!(
                                "\x1b[31m  ✗ PULL \x1b[0m  {} \x1b[90m[{}] ({})\x1b[0m",
                                display, info.branch, e
                            );
                            errors += 1;
                        }
                    }
